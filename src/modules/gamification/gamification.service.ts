...
export class GamificationService {
  ...
  async assignNextMission(userId: string): Promise<UserMissionResponseDto | null> {
    try {
      ...
      console.error('❌ [GAMIFICATION] Erro ao atribuir próxima missão:', error);
      return null;
    }
  }
  ...
  async processClassCompletion(userId: string, classId: string): Promise<void> {
    ...
    try {
      ...
      this.logger.log(`✅ [GAMIFICATION] ===== PROCESSAMENTO DE CONCLUSÃO FINALIZADO COM SUCESSO =====`);
      
    } catch (error) {
      this.logger.error(`❌ [GAMIFICATION] Erro ao processar conclusão de aula:`, error);
