...
export class WebhooksService {
  ...
  async handlePaymentUpdated(payment: any): Promise<void> {
    ...
    try {
      ...
      await db
        .update(payments)
        .set({
          status: this.mapMercadoPagoStatus(payment.status) as any,
          updatedAt: new Date(),
        })
  ...
  async handlePaymentApproved(payment: any): Promise<void> {
    ...
    try {
      ...
      // Atualizar para authorized (custódia)
      await db
        .update(payments)
        .set({
          status: 'authorized',
      ...
      this.logger.log(`✅ [WEBHOOK] Pagamento aprovado e em custódia: ${payment.id}`);
