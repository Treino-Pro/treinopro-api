...
export class WebhooksController {
  ...
  async handleMercadoPagoWebhook(
    ...
    try {
      ...
      const isValid = await this.webhooksService.validateWebhookSignature(payload, headers);
      if (!isValid) {
  ...
  private async handlePaymentWebhook(action: string, paymentId: string): Promise<void> {
    this.logger.log(`💳 [WEBHOOK] Processando pagamento: ${action} - ${paymentId}`);
    ...
    try {
      ...
      if (!payment) {
        this.logger.error(`❌ [WEBHOOK] Pagamento não encontrado: ${paymentId}`);
        return;
      }

      this.logger.log(`✅ [WEBHOOK] Pagamento encontrado: ${payment.id}`);
      this.logger.log(`🔍 [WEBHOOK] Status: ${payment.status}`);
      this.logger.log(`🔍 [WEBHOOK] External Reference: ${payment.external_reference}`);
      ...
      switch (action) {
        ...
        default:
          this.logger.warn(`⚠️ [WEBHOOK] Ação de pagamento não tratada: ${action}`);
      }

    } catch (error) {
      this.logger.error(`❌ [WEBHOOK] Erro ao processar pagamento ${paymentId}:`, error);
      throw error;
    }
  }

  private async handlePlanWebhook(action: string, planId: string): Promise<void> {
  ...
  private async handleSubscriptionWebhook(action: string, subscriptionId: string): Promise<void> {
    this.logger.log(`🔄 [WEBHOOK] Processando assinatura: ${action} - ${subscriptionId}`);
    // Implementar se necessário
  }

  private async handleInvoiceWebhook(action: string, invoiceId: string): Promise<void> {
    this.logger.log(`📄 [WEBHOOK] Processando fatura: ${action} - ${invoiceId}`);
    // Implementar se necessário
  }
}
