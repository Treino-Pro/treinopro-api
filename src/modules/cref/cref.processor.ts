...
import { CrefValidationJobData, CrefValidationJobResult } from './cref-queue.service';
...
export class CrefProcessor {
  ...
  async handleCrefValidation(
    ...
    this.logger.log(`🔄 [PROCESSOR] Processando validação CREF: ${crefNumber} (tentativa ${retryCount + 1})`);
    ...
    try {
      ...
      this.logger.error(`❌ [PROCESSOR] Erro na validação CREF ${crefNumber}:`, error.message);
      
      // Se for um erro de formato, não tenta novamente
      if (error.message.includes('Formato de CREF inválido')) {
        this.logger.warn(`⚠️ [PROCESSOR] CREF ${crefNumber} tem formato inválido, não será reprocessado`);
      ...
      if (error.message.includes('deve ser BACHAREL')) {
        this.logger.warn(`⚠️ [PROCESSOR] CREF ${crefNumber} tem graduação inválida, não será reprocessado`);
        throw error; // Não tenta novamente
      }
